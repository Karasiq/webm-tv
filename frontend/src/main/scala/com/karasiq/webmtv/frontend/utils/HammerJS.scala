...
private class HammerRecognizer(options: js.Object = ???) extends js.Object {
  def set(options: js.Object): Unit =
    js.native
}
...
private class Hammer(element: Element, options: js.Object = ???) extends js.Object {
  ...
  def get(name: String): HammerRecognizer =
    js.native
}
...
object Hammer extends js.Object {
  // Directions
  val DIRECTION_NONE, DIRECTION_LEFT, DIRECTION_RIGHT, DIRECTION_UP, DIRECTION_DOWN, DIRECTION_HORIZONTAL,
      DIRECTION_VERTICAL, DIRECTION_ALL: Int = js.native
}

case class HammerJS(options: Map[String, js.Object] = Map.empty, events: Map[String, Seq[Element ⇒ Unit]] = Map.empty)
    extends Modifier {
  ...
  // Options
  def enable(recognizer: String): HammerJS =
    option(recognizer, "enable" → true)

  def direction(recognizer: String, direction: Int): HammerJS =
    option(recognizer, "direction" → direction)
