...
import org.scalajs.dom.{document, window, Element, KeyboardEvent}
import rx._
      ...
      VideoJSBuilder()
        ...
        .ready { player ⇒
          ...
          def downloadVideo(): Unit = {
            val anchor =
              a(href := videoSource.now.getOrElse("#"), "download".attr := "", target := "_blank", display.none).render
          ...
          // Player buttons
          player.addButton("Previous video", "fast-backward".fontAwesome(), showPrevious.reactiveShow, vjsXsHide)(
            previousVideo()
          )
          ...
          span(
            ...
            Popover(
              ...
              div(
                ...
                ul(
                  for (boardUrl ← boards.map("#" + _))
                    yield li(a(href := boardUrl, boardUrl))
                ),
